...
zipname="KERNEL-KNIGHTRC-NON-KSU.zip"
...
git clone https://github.com/Kyvangka1610/AnyKernel3.git $(pwd)/../AnyKernel3/
fi
}
...
zip_function() {
    if [[ -f out/arch/arm64/boot/Image.gz-dtb ]] && [[ -f out/arch/arm64/boot/dtbo.img ]] ; then
        ...
        cp out/arch/arm64/boot/dtbo.img $(pwd)/../AnyKernel3/
        cd $(pwd)/../AnyKernel3/ && zip -r9 $zipname *
        mv $zipname "$OLDPWD"
        cd -
        sleep 1
        echo
